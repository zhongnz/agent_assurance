...
The kit contains eight markdown templates corresponding to the methodology's named outputs:
...
| `deployment_specification_template.md` | Per-deployment specification (architecture, tools, principal model, authority) | §5.2; mirrors `reference_application/deployment_specification.md` |
| `threat_register_template.md` | Structured threat enumeration organised by OWASP ASI + cross-cutting patterns | §3 / §5.3; mirrors `reference_application/threat_register.md` |
| `gap_analysis_template.md` | Control-by-control evaluation against the v1.0 control matrix | §5.3 / §5.4; mirrors `reference_application/gap_analysis.md` |
...
| `supervisory_engagement_pack_template.md` | Consolidated supervisory-engageable repository | HC-01 |
| `evidence_capture_checklist.md` | Runtime evidence-capture requirements | AT-01, AT-02, AT-03 |
...
2. **Deployment specification** for the agent under assessment.
3. **Threat register** against that deployment.
4. **Gap analysis** of the deployment against the v1.0 control matrix.
5. **Findings** for each gap warranting documentation.
6. **Residual-risk acceptance** per finding (or rolled-up acceptance per deployment).
7. **Evidence-capture inventory** and gaps (AT-01 specifically).
8. **Supervisory engagement pack** consolidating the above.
