...
The toxic-flow pattern is well-evidenced in disclosed agent-security incidents. EchoLeak (CVE-2025-32711, June 2025) is the canonical case: four security guardrails (XPIA classifier, link redaction, image redaction, Content Security Policy) and one UI provenance signal (reference-mention markers) each operated as designed, and the chain composing across them produced the exfiltration. AgentFlayer (Black Hat USA, 6 August 2025) demonstrated comparable patterns across multiple agent configurations. Shadow Escape (22 October 2025) demonstrated further variants. The methodology's case-study analysis of EchoLeak (`case_studies/echoleak.md`) treats the toxic-flow analysis as the methodology output most directly suited to catching such patterns prospectively.
